...
def test_create_item(client, token):
    ...
    response = client.post('/items', headers={'x-access-token': token}, json={
        'name': 'Item Teste'
    })
    assert response.status_code == 201
...
def test_get_item_by_id(client, token):
    ...
    client.post('/items', headers={'x-access-token': token}, json={
        'name': 'Item Teste'
    })
...
def test_update_item(client, token):
    ...
    response = client.put('/items/1', headers={'x-access-token': token}, json={
